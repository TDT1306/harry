...
//levle 5
//bai 3: nhap vao 1 so nguyen duong n. dem so nguyen to tu 1 den n.
...
int demso(int n) // dem so nguyen to tu 1 den n
{
    ...
    {
        if (nguyento(i)) // neu i la so nguyen ti thi tang bien dem len 1 
        {
...
bool nguyento(int n) // liem tra so nguyen to
{
    ...
    else // nguoc lai khong phai la so nguyen to
    {
        return false;
    }
}